...
class Experience1b(Trainer):
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
        self.batch_call = throttle(lambda x: print('{data[acc]:.4f} % {data[loss]:.4f}'.format(data=x)), 10)

        self.classify_fc = nn.Linear(512 * self.model.block.expansion, 1000)
        self.rectify_fc = nn.Linear(512 * self.model.block.expansion, 224 * 224)
    ...
    def train_batch(self):
        ...
        self.chrono.start()
        output = self.model(x)
        loss = self.criterion(self.classify_fc(output), y.long()) + 0.5 * (self.rectify_fc(output) - x) ** 2
...
if __name__ == '__main__':
    ...
    model = utils.enable_cuda(
        TruncatedResNet(resnet.BasicBlock, [2, 2, 2, 2])
    )
